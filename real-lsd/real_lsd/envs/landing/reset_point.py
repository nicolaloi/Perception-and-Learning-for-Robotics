...
class ResetPoint():
    def __init__(self, setting, type, init_pose):
        ...
        if self.reset_type == 'testpoint':
            ...
            log.warn("Reset Module initiailised tihe reset_type: RANDOM")
            self.reset_area = setting['reset_area']
            self.mesh_angle = setting['mesh_angle']
    ...
    def reset_random(self):
        x1 = random.uniform(self.reset_area[0], self.reset_area[1])
        y1 = random.uniform(self.reset_area[2], self.reset_area[3])
        z1 = random.uniform(self.reset_area[4], self.reset_area[5])
        
        a = np.deg2rad(self.mesh_angle)
        x =  np.cos(a)*x1 + np.sin(a)*y1
        y = -np.sin(a)*x1 + np.cos(a)*y1

        idx = self.nearest_point_idx(x,y)

        z = self.points[idx,2] + z1
