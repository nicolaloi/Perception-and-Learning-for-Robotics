...
def generator_function(somedata):
    for i, obj in enumerate(somedata):
        yield (i, (obj[0], obj[1], obj[0], obj[1]), None)
...
class UnrealCvLanding_base(gym.Env):
    ...
    def __init__(self,
        ...
        p.overwrite = False
        #timestamp = str(time())
        #p.dat_extension = timestamp+ 'data'
        #p.idx_extension = timestamp+'index'
        self.rtree_points = rtree.index.Index("plr_points", generator_function(self.points), properties=p) 
        #for i, point in enumerate(xyz):
        #    index_pred.insert(i, (point[0], point[1], point[2], point[0], point[1], point[2]))

        self.roughness             = points_list[:,3]
        self.slope                 = points_list[:,4]
        ...
        for i in range(self.points.shape[0]):
            if self.points[i,0]>=reset_area[0] and self.points[i,0]<=reset_area[1] and self.points[i,1]>=reset_area[2] and self.points[i,1]<=reset_area[3]:
                n_points+=1
                if self.landable[i]:
                    n_land_points += 1
    ...
    def nearest_point_idx(self, x, y):
        ...
        #rtree_time = tmr()
        idx = list(self.rtree_points.nearest((x, y, x, y), 1))
        #print("\n rtree:", tmr() - rtree_time, "idx:", idx, "\n")
        return idx[0]
    ...
    def _step(self, action ):
        ...
        if self.observation_type in ['Color', 'Rgbd', 'PoseColor', 'Features', 'StepHeightFeatures', 'StepHeightVelocityFeatures']:
            info['Color']  = self.unrealcv.img_color

        if self.observation_type in ['Depth', 'Rgbd']:
            info['Depth']  = self.unrealcv.img_depth

        if self.observation_type in ['Features', 'StepHeightFeatures', 'StepHeightVelocityFeatures']:
        ...
        #log.warn(" Nearest point x,y: {}".format((self.points[idx,0],self.points[idx,1])))
        ...
        if 'mask' in self.reward_type: # and not info['Collision']:
            ...
            rew, done, succ, mesh_dist, out_of_boundaries = self.reward_function.reward_mask_height(info['Trigger'],
                                                                    info['Pose'], error, mesh_height,
